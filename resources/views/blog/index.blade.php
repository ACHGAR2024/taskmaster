...
<body class="font-sans antialiased bg-gray-900">
    <div class="min-h-screen bg-arpp dark:bg-arpp">
        ...
        <main class="bg-gray-100 bg-grisp text-blancp">
            <!-- Page Content -->
            <div class="max-w-7xl mx-auto pt-6 px-4 sm:px-6 lg:px-8 text-center">
                <button class="px-6 py-2 text-blancp bg-bleup rounded hover:bg-vertp">
                    <a href="{{ route('blogs.create') }}" class="block w-full h-full">
                        {{ __('Créer un post') }}
                    </a>
                </button>
            </div>
                        ...
                        <div class="w-full lg:w-8/12">
                            ...
                            @foreach ($blogs as $blog)
                                <div class="mt-6 rounded-md">
                                    <div class="max-w-4xl px-10 py-6 mx-auto bg-white  shadow-md  bg-vertp rounded-md">
                                        {{-- {{ $blog->image }}   <img src="/storage/uploads/{{ $blog->image }}"
                                            class="w-full h-64 object-cover rounded-t-lg" alt="Post Image"> --}}
                                        <a href="{{ route('blogs.show', $blog) }}"><img
                                                src="https://picsum.photos/700/500?random={{ $blog->id }}"
                                                alt="Random Image 1" class="w-full h-64  rounded-t-l object-cover"></a>
                                        <div class="mt-2">
                                            <a href="{{ route('blogs.show', $blog) }}"
                                                class="text-2xl font-bold text-gray-700 hover:underline">{{ $blog->title }}</a>
                                            <p class="mt-2 text-gray-600">{{ Str::limit($blog->content, 120) }}</p>
                                        </div>
