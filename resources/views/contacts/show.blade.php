...
@section('content')
    ...
    <div class="container mx-auto px-4 py-8 bg-grisp text-blancp">
            ...
            @else
                @if ($currentUser->role_id == 2)
                    <table class="table-auto w-full">
                        <thead class="bg-vertp text-white">
                            <tr>
                        ...
                        <tbody>
                            @foreach ($contacts as $contact)
                                <tr class="{{ $loop->even ? 'bg-grisp' : 'bg-blancp text-noirp' }}">
                                    ...
                                    <td class="border px-4 py-2 text-sm">
                                        <form action="{{ route('contacts.destroy', $contact->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="bg-rougep text-blancp hover:bg-vertp text-white font-bold py-1 px-2 rounded">
                                                Supprimer
                                            </button>
                                        </form>
                                    </td>
