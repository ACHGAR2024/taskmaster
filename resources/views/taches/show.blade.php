...
<body class="font-sans antialiased bg-arpp">
    <div class="min-h-screen bg-slate-900 dark:bg-gray-900">
        ...
        @endisset
        <main class="bg-grisp h-screen">
            <div class="container mx-auto p-4">
                <h1 class="text-2xl font-bold mb-4 text-blancp">Détails de la Tâche</h1>

                <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
                    <div class="md:flex bg-arpp">
                        <div class="md:flex-shrink-0">
                            <img class="h-48 w-full object-cover md:w-48" src="https://picsum.photos/150"
                                alt="Tâche">
                        </div>
                        <div class="p-8">
                            <div class="uppercase tracking-wide text-sm text-indigo-500 font-semibold">Nom</div>
                            <p class="mt-2 text-gray-600">{{ $tache->name }}</p>
                            <div class="uppercase tracking-wide text-sm text-indigo-500 font-semibold mt-4">Description
                            </div>
                            <p class="mt-2 text-gray-600">{{ $tache->description }}</p>
                            <div class="uppercase tracking-wide text-sm text-indigo-500 font-semibold mt-4">Colonne
                            </div>
                            <p class="mt-2 text-gray-600">
                                {{ $tache->column ? $tache->column->name : 'Aucune colonne associée' }}</p>
                            <div class="mt-6 flex justify-end">
                                <a href="{{ route('taches.index') }}"
                                    class="inline-block mr-2 bg-bleup text-blancp shadow-md px-4 py-2 rounded hover:bg-grisp">Retour</a>
                                <a href="{{ route('taches.edit', $tache->id) }}"
                                    class="inline-block bg-bleup text-blancp shadow-md px-4 py-2 rounded hover:bg-grisp">Éditer</a>
                            </div>
                        </div>
                    </div>
                </div>
