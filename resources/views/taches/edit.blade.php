    ...
    <div class="min-h-screen bg-slate-900 dark:bg-gray-900">
        ...
        <main class="bg-grisp h-screen">
            <div class="container mx-auto p-4">
                <h1 class="text-2xl font-bold mb-4 text-blancp">Éditer la Tâche</h1>
                ...
                <form action="{{ route('taches.update', $tache->id) }}" method="POST"
                    class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 bg-blancp">
                    @csrf
                    @method('PUT')
                        ...
                        <select
                            ...
                            @foreach ($columns as $column)
                                <option value="{{ $column->id }}"
                                    {{ $column->id == $tache->column_id ? 'selected' : '' }}>
                                    {{ $column->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex items-center justify-between">
                        <button type="submit"
                            class="bg-bleup hover:bg-grisp text-blancp font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Mettre
                            à jour</button>
                        <a href="{{ route('taches.index') }}"
                            class="bg-bleup hover:bg-grisp text-blancp font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Retour</a>
                    </div>
                </form>
